...
object TerminateProductWS {
  ...
  def execute(eventSource: String, productIdentifier: String, locale: Locale) = {
    ...
    try {
      PostpaidServiceConfig.getWs.terminateProduct(eventSource, productIdentifier, locale.getLanguage,
        PostpaidServiceConfig.getApiUserName, PostpaidServiceConfig.getApiPassword, responseCode, responseMessage)
    } catch {
      case e: Exception =>
        if (log.isErrorEnabled()) {

          val errorMessage = new StringBuilder("caught [Exception] in [TerminateProductWS] with message [")
        ...
        responseMessage.value = new ApplicationApiResponsesDAO().getResponseText(ApplicationConstants.APPLICATION_NAME, ApplicationConstants.TerminateProductWS, 1, locale)
    } finally {

      val inputObject = new StringBuilder("TerminateProductWS inputs eventsource[")
      ...
      appLog.setApiCalled(ApplicationConstants.TerminateProductWS)
    ...
    responseMessage.value = if (responseCode.value == 0) {
      new ApplicationApiResponsesDAO().getResponseText(ApplicationConstants.APPLICATION_NAME, ApplicationConstants.TerminateProductWS, 0, locale)
    } else {
